...
description: 🛡️ 三因子ETF国家队资金监测系统 v7.1 — 追踪国家队（中央汇金）ETF操作信号。零第三方依赖（移除akshare）。数据源：腾讯财经K线(HTTP) + 东方财富实时份额API。支持：① 数据获取 ② 本地SQLite存档 ③ 三因子分析（量能P50% + 方向P20% + 份额P30%）④ HTML可视化报告 ⑤ 邮件发送（支持QQ邮箱/任意SMTP）。当用户需要运行ETF三因子分析、查询国家队信号、查看每日监测报告、或设置定时任务时使用。
---

# 🛡️ etf-three-factor-v7 — 三因子ETF监测系统

> **v7.1 升级**: 移除 akshare 依赖，切换为东方财富实时API（f84流通份额字段），零第三方库，SSL安全修复，邮件变量统一为 ETF_SMTP_PASS。
...
| 数据 | API | 回溯能力 |
|------|-----|----------|
| K线行情 | `web.ifzq.gtimg.cn` (HTTP) | 60天历史，无需SSL |
| ETF份额 | `push2.eastmoney.com` f84字段 | ✅ 实时盘中/盘后，本地DB存档历史 |
| 无第三方依赖 | 仅Python标准库 | ✅ 无需 pip install |
...
export ETF_SMTP_PASS="你的16位授权码"  # 统一使用此变量名

# 永久写入 ~/.zshrc（推荐）
echo 'export ETF_EMAIL_FROM="你的邮箱@qq.com"' >> ~/.zshrc
echo 'export ETF_EMAIL_TO="你的邮箱@qq.com"' >> ~/.zshrc
echo 'export ETF_SMTP_PASS="你的授权码"  # v7.1 统一变量名（移除旧版 QQMAIL_AUTH_CODE/SMTP_PASS）' >> ~/.zshrc
source ~/.zshrc
```
...
- Python 3.7+
- **零第三方依赖** — 仅 Python 标准库（json, urllib, ssl, sqlite3, smtplib, email, argparse）
- Python 3.7+
